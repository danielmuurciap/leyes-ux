---
name: leyes-ux
description: "Úsala siempre que vayas a crear o cambiar interfaz — una pantalla, un componente, un formulario, una lista, botones, navegación entre vistas, filtros, estados de carga, vacío o error — en React, Vue, HTML, SwiftUI o cualquier stack, y antes de darla por terminada. Convierte las 30 leyes de UX (Fitts, Hick, Jakob, Doherty, Von Restorff…) en decisiones: qué componente va, qué número hay que cumplir y qué se comprueba solo, separando lo universal de lo que depende del producto. Incluye un gate para React (JSX/TSX). No sirve para backend."
license: MIT
---
...
2. **Construir.** Cumple los presupuestos duros. Si un contextual no aplica,
   escribe por qué.
3. **Comprobar y declarar.** Corre el gate si el proyecto usa JSX/TSX, vuelve a
   pasar la fila de la superficie contra lo construido y **entrega este bloque
   junto con el código**. Sin él, la pantalla no está terminada:

   ```text
   Leyes UX · <pantalla>
   Tipo: <tarea | comparación | panel | configuración | exploración | lienzo>
   El usuario viene a: <una línea> · Si se equivoca: <qué pasa>
   Superficies: <formulario, lista, estado vacío…>
   Duras: objetivos ≥ 44 px <✓ | ✗ archivo:línea> · feedback < 400 ms <✓ | ✗> · sin botones sueltos para cambiar de vista <✓ | ✗>
   Contextuales: <presupuesto>: <cumple | no aplica: motivo>   (una por línea, solo las de tus superficies)
   Gate: <resumen de check-leyes | no aplica: el proyecto no usa JSX/TSX>
   ```

   Un ✓ que no has comprobado es peor que un ✗. Si no lo has comprobado,
   escribe «sin comprobar».
...
Si la skill está instalada como plugin de Claude Code, un hook corre el gate
cada vez que editas un `.jsx` o `.tsx` y te devuelve las reglas duras que
incumple. Corrígelas en ese momento, no al final.
...
| **Lista / tabla** | Fragmentación · Posición en serie · Tesler | Si son muchos elementos homogéneos, agrúpalos, ordénalos o añade búsqueda para que se puedan recorrer. Lo importante al principio y al final; lo secundario en medio. Siempre hay rama de lista vacía. |
| **Navegación entre vistas** | Jakob · Modelo mental · Región común | Si cambia lo que se VE, es `Tabs` o `SegmentedControl`. **Nunca botones sueltos, ni siquiera dos.** Ver `references/componentes.md`. |
...
Si la pantalla compara, es un panel de control, de configuración o de
exploración, **no fuerces el umbral**: anota por qué no aplica. Esa nota es la
decisión, y vale más que el número. En un lienzo casi ninguno aplica. En una
pantalla de tarea aplican todos.
...
| Botones sueltos para cambiar de vista | 0 | Jakob | un botón que navega miente sobre lo que hace. El gate solo lo detecta a partir de 3 valores distintos |
...
| Opciones entre las que hay que elegir, sin agrupar | ≤ 7 | Hick | configuración, catálogos, exploración |
...
```bash
node <carpeta de esta skill>/scripts/check-leyes.mjs [--report] [--estricto] [--json] <ruta>
```
...
aplica: usa las tablas a mano. Cada aviso sale como `archivo:línea`.

| Regla | Capa | Qué detecta |
|---|---|---|
| `objetivo-pequeno` | dura | `<button>`, `<a>`, `<Button>`, `<IconButton>`, `<Link>`… cuya propia etiqueta tiene una altura por debajo de 44 px: `h-9`, `size-8`, `h-[40px]` o un token `--spacing-*` del CSS del proyecto. No cuenta las clases de los hijos, las de escritorio (`sm:`, `md:`…) ni las que estilan a otro elemento (`[&_svg]:size-4`). Un `min-h` ≥ 44 px gana |
| `botones-en-vez-de-tabs` | dura | 3 o más `onClick={() => setX(valor)}` sobre el mismo estado con valores distintos (`"dia"`, `"semana"`, `"mes"`). Abrir y cerrar (`true`/`false`) no cuenta |
| `async-sin-estado` | dura | un `await` en un fichero con `onClick`, `onSubmit` o `action={…}` y sin `isLoading`, `isPending`, `useFormStatus`, `useOptimistic`, `Suspense`, `Skeleton`… Un server component sin interacción no cuenta |
| `primario-multiple` | contextual | más de un `variant="primary"` o `variant="default"` |
| `acento-sin-racionar` | contextual | más de 4 clases `bg-`, `text-`, `border-`, `ring-`, `from-` o `to-` con `primary`, `brand` o `accent` (`text-primary-foreground` no cuenta) |
| `lista-sin-vacio` | contextual | un `.map(` sin rama para la lista vacía |

Sale con `0` si no hay duras, `1` si las hay y `2` si el uso es incorrecto: un
flag desconocido, una ruta que no existe o una ruta sin ningún `.jsx`/`.tsx`.
`--estricto` hace que las contextuales también fallen, para cuando el equipo ya
decidió el contexto. `--report` no falla nunca. `--json` devuelve
`[{ fichero, linea, regla, capa, ley, detalle }]`.

**Excepción:** `// leyes:allow <id> <motivo>` (en JSX,
`{/* leyes:allow <id> <motivo> */}`), todo en la misma línea. **Sin motivo no
cuenta.** En `objetivo-pequeno` vale para el elemento: en la línea donde empieza
o en la siguiente a su cierre. En las demás reglas vale para el fichero entero.
En las contextuales el motivo es lo único que importa: «este panel compara dos
periodos, los dos primarios son el diseño».

**Hook.** Instalado como plugin de Claude Code, el plugin ejecuta
`check-leyes.mjs --hook` después de cada `Edit` o `Write` sobre un `.jsx` o
`.tsx` y devuelve al agente las reglas duras que incumple. Con `npx skills` o
copiando la carpeta no hay hook: el gate se corre a mano.

Es heurístico: lee texto, no renderiza. Ante la duda, las reglas duras callan.
Antes de fiarte de él en un proyecto, provoca un fallo a propósito y comprueba
que sale en rojo. Un gate que nunca has visto fallar no sabes si funciona.
...
- **Tokens, contraste y accesibilidad completa.** Necesitan sus propias
  herramientas: `eslint-plugin-jsx-a11y` mientras escribes, y axe-core o
  Lighthouse sobre la página renderizada.
